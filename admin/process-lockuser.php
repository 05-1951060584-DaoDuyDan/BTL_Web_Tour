...
if (isset($_SESSION['LoginOK']) && substr($_SESSION['LoginOK'], 0, 1) == '3') {
    if(isset($_GET['id'])){
        $sql = "UPDATE tb_user set `lock` = 1 where id_user = {$_GET['id']}";
        ...
        $sql1 = "UPDATE tb_touroperator set `lock` = 1 where id_touroperator = {$id_Manager}";
        $sql2 = "UPDATE tb_tour set `lock` = 1 where id_touroperator = {$id_Manager}";
