...
if (!isset($_SESSION['LoginOK']) && !substr($_SESSION['LoginOK'], 0, 1) == '1') {
    ...
    if (isset($_POST)) {
        ...
        $stmt = $dbh->prepare("UPDATE `tb_tourday` SET `location` = ?, `morning` = ?, `noon` = ?, `afternoon` = ?, `night` = ?, `imgday` = ? where `day` = ? and `tour_code` = ?");
        ...
        $stmt->bindParam(7, $dayupdate);
        $stmt->bindParam(8, $tourco);
        if ($stmt->execute()) {
            $sql1 = "SELECT day, location, morning, noon, afternoon, night, numberofdays, imgday from tb_tourday, tb_tour where tb_tour.tour_code = '{$tourco}' and tb_tourday.tour_code = tb_tour.tour_code";
