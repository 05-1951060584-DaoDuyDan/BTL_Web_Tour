...
<head>
    ...
    <title>Xác thực Email Người dùng</title>
    <!-- CSS -->
...
<body>
    <?php
        if($_GET['key'] && $_GET['token']) //nó bắt giá trị trên url
        {
            ...
            $email = $_GET['key'];
            $token = $_GET['token'];
            $query = mysqli_query($conn, "SELECT * FROM `tb_user` WHERE `email_verification_link`='".$token."' and `email`='".$email."';"
        );
        ...
        if (mysqli_num_rows($query) > 0) { //có bản ghi nào khớp với thông tin không?
            ...
            if($row['email_verified_at'] == NULL){
                mysqli_query($conn,"UPDATE tb_user set email_verified_at ='" . $d . "', status_user = 1 WHERE email='" . $email . "'");
